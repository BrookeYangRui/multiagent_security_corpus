...
Every active work appears exactly once under `papers/` and in `papers/index.csv`. Current dominant-contribution totals are **45 attacks, 80 defenses, 45 evaluations, 12 general works, and 7 surveys**.
...
Run `scripts/validate_all.sh`. Validation must enforce Set 1 = 92, Set 2 = 97, total = 189, the 45/80/45/12/7 contribution partition, exact paper-index membership, and exactly 187 non-README paper notes.
